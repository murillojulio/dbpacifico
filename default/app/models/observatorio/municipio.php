...
//    }
     ...
     * @return type
     */
    public function getListadoMunicipio($estado='todos', $order='', $page=0) {                   
        $columns = 'municipio.*, departamento.nombre AS departamento';        
        $join = 'INNER JOIN departamento ON departamento.id = municipio.departamento_id';
        $conditions = 'municipio.id IS NOT NULL AND municipio.id =! 0'; 
        ...
        if($page) { 
            ...
            return $this->paginated_by_sql('SELECT municipio.*, subregion.nombre AS subregion, departamento.nombre AS departamento,
                ...
                WHERE municipio.id IS NOT NULL AND municipio.id != 0 GROUP BY municipio.nombre ORDER BY '.$order, "page: $page");
