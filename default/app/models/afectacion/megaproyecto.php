...
class Megaproyecto extends ActiveRecord {    
    ...
    public function getListadoMegaproyecto($estado='todos', $order='', $clase_megaproyecto='', $page=0) {                   
        ...
        $order = $this->get_order($order, 'nombre', array(            
            ...
            'monto_inversion' => array(
                'ASC' => 'megaproyecto.monto_inversion ASC, megaproyecto.monto_inversion ASC',
                'DESC' => 'megaproyecto.monto_inversion DESC, megaproyecto.monto_inversion DESC'
            )));
        
        if($page) { 
            
            return $this->paginated_by_sql("SELECT megaproyecto.* FROM megaproyecto WHERE megaproyecto.id IS NOT NULL AND megaproyecto.clase_megaproyecto ='".$clase_megaproyecto."' GROUP BY megaproyecto.nombre ORDER BY ".$order, "page: $page");
            /*   
             *             return $this->paginated_by_sql("SELECT megaproyecto.*, tipo_megaproyecto.nombre AS tipo_megaproyecto FROM megaproyecto
                INNER JOIN tipo_megaproyecto ON tipo_megaproyecto.id = megaproyecto.tipo_megaproyecto_id                
WHERE megaproyecto.id IS NOT NULL AND megaproyecto.clase_megaproyecto ='".$clase_megaproyecto."' GROUP BY megaproyecto.nombre ORDER BY ".$order);
             */
        }
